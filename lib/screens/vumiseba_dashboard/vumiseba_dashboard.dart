...
import 'package:vumi_seba/custom_http/custom_http.dart';
...
class _VumiSebaDashboardState extends State<VumiSebaDashboard> {
  ...
  void getTokenAndSave() async {
    try {
      String token = await CustomHttpRequest.getToken('mysoft', '#mysoft@ldtax#');
      await saveToken(token);
      //Navigator.pushNamed(context, '/nextPage', arguments: token);

     await Navigator.of(context).push(MaterialPageRoute(builder: (context) => VumiSebaiKormokorta(
        token: token,
      ),));
    } catch (e) {
      // handle error
